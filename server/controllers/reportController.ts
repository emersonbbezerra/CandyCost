...
export const getReports = async (_req: Request, res: Response) => {
  try {
    ...
    const profitabilityAnalysis = productCosts.map(({ product, cost }) => {
      const profitMargin = (typeof (cost as any).suggestedPrice === 'number' && (cost as any).suggestedPrice > 0) 
        ? (((cost as any).suggestedPrice - cost.totalCost) / (cost as any).suggestedPrice) * 100 
        : 0;
